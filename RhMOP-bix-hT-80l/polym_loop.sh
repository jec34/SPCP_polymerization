...
bonds=151         # initial number of bonds (0 unless restart)
bondsTotal=200    # total number of bonds to form
bondsCycle=8     # bonds to form per cycle
...
function energyMin
{
	...
	mpirun -np 8 lmp_mpi -in min.in > out
	[[ -a log.lammps && -a out ]] \
...
function molDyn
{
	...
	mpirun -np 8 lmp_mpi -in md.in > out
	[[ -a log.lammps && -a out ]] \
